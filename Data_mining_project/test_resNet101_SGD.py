...
def create_datasets_and_loaders(df, series_description, batch_size=8):
...
class CustomResNet(nn.Module):
    def __init__(self, num_classes=2, pretrained=True):
        ...
        self.model = models.resnet101(pretrained=pretrained)  # Use pre-trained weights
...
# Initialize models
sagittal_t1_model = CustomResNet(num_classes=3).to(device)
axial_t2_model = CustomResNet(num_classes=3).to(device)
sagittal_t2stir_model = CustomResNet(num_classes=3).to(device)
...
optimizer_sagittal_t1 = torch.optim.Adam(sagittal_t1_model.model.parameters(), lr=0.001, weight_decay=1e-4)
optimizer_axial_t2 = torch.optim.Adam(axial_t2_model.model.parameters(), lr=0.001, weight_decay=1e-4)
optimizer_sagittal_t2stir = torch.optim.Adam(sagittal_t2stir_model.model.parameters(), lr=0.001, weight_decay=1e-4)
...
scheduler = torch.optim.lr_scheduler.StepLR(optimizer_sagittal_t1, step_size=7, gamma=0.1)
